...
즉, 리소스가 어디에 “위치”하는지를 알려줄 뿐이죠.

> 하지만 만약 리소스가 다른 서버로 옮겨가 버린다면?\
> → 기존 URL은 더 이상 동작하지 않ㄴ ❌
...
그래서 연구자들은 “위치와 무관하게 변하지 않는 이름”을 고민했어요.\
그게 바로 **URN (Uniform Resource Name)** 입니다.

* 예: `urn:ietf:rfc:2141`\
  이건 RFC 문서 2141을 가리키는 이름이에요.\
  어디에 저장돼 있든, 몇 군데 복사돼 있든 **이름 자체로 동일한 리소스**를 식별할 수 있죠.

***

### 6️⃣ PURL – 임시 다리

URN이 본격적으로 자리 잡기 전까지는 \*\*PURL (Persistent URL)\*\*이라는 방식이 쓰였어요.
...
👉 즉, \*\*“내가 직접 바뀌는 주소를 기억할 필요 없이, 항상 PURL만 부르면 된다”\*\*는 개념이에요.
