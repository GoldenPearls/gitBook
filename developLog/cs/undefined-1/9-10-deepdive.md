...
이전에 여러 자바스크립트 엔진이 사용되고 있었지만 다른 자바스크립트 엔진은 웹 특성상 유저와 상호작용을 위해 즉시성이 있는 인터프리터 방식을 사용하는데, 이는 코드가 많아질수록 속도가 느려진다는 단점이 있습니다. 이 점을 보완해서 속도 향상을 위해 **v8엔진은 인터프리터를 사용하는 대신 JIT(Just In Time) 컴파일러**를 구현함으로써 코드 실행 시에 **자바스크립트 코드를 머신 코드로 컴파일**합니다.
...
1. **Parser**: JS 코드를 토큰(token)으로 나눔
2. **AST(Abstract Syntax Tree)** 생성
3. **Ignition (인터프리터)**: AST를 바탕으로 **Bytecode** 생성 후 실행 시작
4. **Profiler**: 반복 사용되는 코드를 분석 (Hot Spot 탐지)
5. **TurboFan (최적화 컴파일러)**: 자주 사용되는 코드를 **기계어로 변환**하여 성능 최적화

#### 중간 코드 - Bytecode

* **Bytecode**란?
  * 고수준 언어(JS)를 가상 머신이 이해할 수 있도록 중간 단계로 변환한 코드
  * 실행 속도를 높이고, 메모리 소비를 줄이며, 파싱 오버헤드를 줄임

#### TurboFan 최적화 컴파일러

* **Profiler**가 Hot Code를 발견하면 TurboFan으로 넘겨 최적화
* 이때 사용되는 최적화 기법:
  * **Hidden Class**: 객체 구조를 추론하고 캐시화
  * **Inline Caching**: 자주 쓰이는 메서드를 인라인 처리

***

### ♻️ Deoptimization (역최적화)

* 런타임 중 타입이 바뀌거나 예상과 다른 실행 경로가 나오면 최적화된 코드를 **다시 Bytecode로 복귀**
...
| 구성 요소    | 설명                          |
| -------- | --------------------------- |
| Ignition | Bytecode 생성 및 해석 실행 (인터프리터) |
| TurboFan | 최적화된 머신코드 생성 (JIT 컴파일러)     |
| Profiler | 실행 중인 코드의 사용 패턴을 수집         |
| Bytecode | 가상머신용 중간 코드                 |
