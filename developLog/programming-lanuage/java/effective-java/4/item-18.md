...
* **상속(Inheritance)**:
  *   **Is-a 관계**를 나타
      ...
      상속 대신 \*\*컴포지션(Composition)\*\*을 사용하여 기존 `Set` 인스턴스를 감싸는 \*\*래퍼 클래스(Wrapper Class)\*\*를 만들어봅니다.

      ```java
      java코드 복사public class InstrumentedSet<E> implements Set<E> {
      ...
      * `InstrumentedSet`은 `Set` 인터페이스를 구현하고, 내부에 실제 작업을 수행할 `Set` 인스턴스를 가집니다.
      * 모든 메서드는 내부 `set` 객체에 작업을 위임합니다.
      * `add()`와 `addAll()` 메서드에서 `addCount`를 정확하게 증가시킬 수 있습니다.
      * 상속이 아닌 컴포지션을 사용함으로써 상위 클래스의 내부 구현에 의존하지 않습니다.

      **사용 예시**:

      ```java
      java코드 복사Set<String> s = new InstrumentedSet<>(new HashSet<>());
      s.addAll(Arrays.asList("틱", "탁탁", "펑"));
      ...
      * **안전성**: 상위 클래스의 내부 구현 변화에 영향을 받지 않습니다.
      * **유연성**: 기존 클래스의 기능을 확장하거나 변경할 때 더 안전하게 구현할 수 있습니다.
      * **재사용성**: 다양한 클래스와 함께 사용할 수 있으며, 기능을 추가하거나 변경하기 쉽습니다.

      **5. 재사용 가능한 포워딩 클래스 작성**

      위임 메서드를 일일이 작성하는 대신, 재사용 가능한 포워딩 클래스를 만들어서 중복 코드를 줄일 수 있습니다.

      ```java
      java코드 복사public class ForwardingSet<E> implements Set<E> {
      ...
      이제 `InstrumentedSet`은 `ForwardingSet`을 상속받아 필요한 기능만 추가하면 됩니다.

      ```java
      java코드 복사public class InstrumentedSet<E> extends ForwardingSet<E> {
          private int addCount = 0;
      ...
      ```
  * 자식 클래스는 부모 클래스의 특성을 그대로 물려받으며, 부모 클래스의 **일종**입니다.
  * 예시: `Smartphone`은 `ElectronicDevice`이다.
* **컴포지션(Composition)**:
  * **Has-a 관계**를 나타냅니다.
...
* **상속**은 **Is-a 관계**를 나타내며, 클래스 간의 계층 구조를 형성합니다. 하지만 내부 구현에 의존하거나 강한 결합이 발생할 수 있으므로 주의해야 합니다.
* **컴포지션**은 **Has-a 관계**를 나타내며, 클래스의 기능을 유연하게 확장하고 재사용할 수 있습니다. 결합도가 낮아 유지보수가 용이하지만, 다형성 활용이 제한적일 수 있습니다.
* **상황에 따라 적절한 방법을 선택**하여 사용하는 것이 중요합니다. 클래스 간의 관계를 명확히 파악하고, 코드의 재사용성과 유지보수성을 고려하여 상속과 컴포지션을 적절히 활용하세요.
...
* **상속은 is-a 관계일 때만 사용**해야 한다. 즉, 하위 클래스가 상위 클래스의 진짜 하위 타입인 경우에만 상속을 사용해야 한다.
* **컴포지션과 위임**을 사용하면 상속의 단점을 피하면서 유연하고 안전하게 기능을 확장할 수 있다.
